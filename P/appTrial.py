...
def show_batch_processing(dashboard):
    ...
    **Process multiple domains for NCIIPC submission analysis.**
    Upload a file containing domains to analyze the ENTIRE dataset.
    """)
    ...
    if domains_to_process:
        ...
        with col1:
            ...
            st.write("All domains will be analyzed for:")
            st.write("• Phishing classification")
            st.write("• CSE entity matching")
            st.write("• Submission eligibility")
        
        with col2:
            st.write("")
            st.write("")
            process_btn = st.button("🚀 PROCESS ALL DOMAINS", type="primary", use_container_width=True)
        ...
        if process_btn:
            ...
            # Display results
            if results:
                display_batch_results(results, len(domains_to_process))

def display_batch_results(results, total_domains_processed):
    ...
    if not eligible_domains.empty:
        ...
        with col1:
            ...
            href_full = f'<a href="data:file/csv;base64,{b64_full}" download="nciipc_complete_analysis.csv" class="button">📥 Download Complete Analysis</a>'
            st.markdown(href_full, unsafe_allow_html=True)
        
        with col2:
            # Eligible domains only
            csv_eligible = eligible_domains.to_csv(index=False)
            b64_eligible = base64.b64encode(csv_eligible.encode()).decode()
            href_eligible = f'<a href="data:file/csv;base64,{b64_eligible}" download="nciipc_eligible_domains.csv" class="button">🚨 Download Eligible Domains</a>'
            st.markdown(href_eligible, unsafe_allow_html=True)
        
        with col3:
            # CSE-targeted only
            csv_cse = cse_targeted.to_csv(index=False)
            b64_cse = base64.b64encode(csv_cse.encode()).decode()
            href_cse = f'<a href="data:file/csv;base64,{b64_cse}" download="nciipc_cse_targeted.csv" class="button">🎯 Download CSE-Targeted</a>'
            st.markdown(href_cse, unsafe_allow_html=True)
    else:
        st.warning("""
...
def create_nciipc_submission_format(submission_data, app_id):
    """Create the NCIIPC submission format"""
    ...
    for i, domain_data in enumerate(submission_data, 1):
        row = {
            ...
            'Source of detection': 'AI Model Prediction',
            ...
            'Date of Post': '',
            'Remarks': f"AI Model Confidence: {domain_data['Phishing_Probability']:.4f}"
        }
